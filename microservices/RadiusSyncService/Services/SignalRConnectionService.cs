...
public class SignalRConnectionService : BackgroundService
{
    ...
    private readonly MachineIdentityService _machineIdentityService;
    ...
    public SignalRConnectionService(
        ...
        MachineIdentityService machineIdentityService)
    {
        _logger = logger;
    ...
    private void RegisterMessageHandlers()
    {
        ...
        _hubConnection.On<object>("RegistrationRejected", (data) =>
        {
            _isRegistered = false;
            ...
            if (status == "pending")
            {
                ...
                _logger.LogWarning("Please contact an administrator to approve this microservice connection");
            }
            else if (status == "revoked")
            {
                _logger.LogError("Registration rejected - connection has been revoked: {Message}", message);
                _logger.LogError("Contact an administrator to restore access");
            }
            else
            {
                _logger.LogWarning("Registration rejected: {Reason} - {Message}", reason, message);
            }
        });
    ...
    private async Task RegisterWithHub()
    {
        ...
        try
        {
            ...
            // If no approval token exists, this is first time - generate one
            if (string.IsNullOrEmpty(approvalToken))
            {
                // Token will be generated by the hub during RequestApprovalAsync
                approvalToken = Guid.NewGuid().ToString(); // Temporary placeholder
            }
            ...
            var metadata = new Dictionary<string, string>
            {
                ...
                { "MachineName", _machineIdentityService.GetMachineName() },
                { "Platform", _machineIdentityService.GetPlatform() }
            };

            await _hubConnection.InvokeAsync("RegisterService", _serviceName, _serviceVersion, machineId, approvalToken, metadata);
            _logger.LogInformation("Registered with hub as {ServiceName} v{Version} - Machine: {MachineId}", _serviceName, _serviceVersion, machineId);
        }
        catch (TaskCanceledException)
        {
            // Registration was likely rejected - the RegistrationRejected event handler will log the details
            _logger.LogDebug("Registration call was canceled - likely due to rejection");
        }
        catch (Exception ex)
        {
